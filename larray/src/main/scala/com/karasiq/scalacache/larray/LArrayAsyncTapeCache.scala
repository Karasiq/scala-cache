...
object LArrayAsyncTapeCache {
  def apply[K](sizeBytes: Long = 1024 * 1024 * 32): LArrayAsyncTapeCache[K] = {
    new LArrayAsyncTapeCache(sizeBytes)
  }
}

class LArrayAsyncTapeCache[K](sizeBytes: Long) extends Cache[K, Future[ByteString]] {
  ...
  protected val entriesMap = mutable.Map.empty[K, Entry]
